...
const Main = () => {
  ...
  const [tasks, setTasks] = useState<{ [key: string]: { text: string; category: 'work' | 'workout' | 'regular'; completed: boolean; notes?: string; priority?: 'low' | 'medium' | 'high' }[] }>({});
  ...
  const colorMap = { work: 'yellow', workout: '#0D1104', regular: '#18563C' };
  ...
  const handleAddTask = async () => {
    if (selectedDate && taskInput.trim()) {
      ...
      const newTask = { text: taskInput, category: taskCategory, completed: false, notes: taskNotes, priority };
  ...
  const getMarkedDates = () => {
    ...
    // Add selected date styling (on top of all other styles)
    if (selectedDate) {
      marked[selectedDate] = {
        ...(marked[selectedDate] || {}),
        selected: true,
        selectedColor: 'lightgrey', // keep transparent to show border
        customStyles: {
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.taskSection}>
        {tasks[selectedDate || '']?.map((task, index) => (
          <Swipeable
            ...
            <View style={[styles.taskCard, { backgroundColor: task.completed ? '#888' : task.category === 'work' ? 'yellow' : task.category === 'workout' ? '#0D1104' : '#184E56' }]}>
              <Text style={[styles.taskText, task.completed && { textDecorationLine: 'line-through' }]}>{task.text}</Text>
              {task.notes ? <Text style={styles.taskNote}>{task.notes}</Text> : null}
            </View>
          </Swipeable>
        ))}
      ...
      <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
        <View style={{ flex: 1, justifyContent: 'center', backgroundColor: 'rgba(0,0,0,0.6)' }}>
          <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ backgroundColor: '#FFF', margin: '5%', padding: '5%', borderRadius: 20, height:"50%" }}>
            ...
            <TextInput style={{ borderBottomWidth: 1, borderBottomColor: '#999', marginBottom: '5%', padding: 8, fontSize: 16, color: '#333' }} placeholder="Enter task" value={taskInput} onChangeText={setTaskInput} placeholderTextColor="#999" />
  ...
  {['work', 'workout', 'regular'].map((cat) => (
    <TouchableOpacity
      ...
      style={{
        ...
        borderColor: taskCategory === cat ? 'blue' : 'transparent',
      }}
    >
      <View
        style={{
      ...
      <Text style={{ color: '#FFF', textTransform: 'capitalize' }}>{cat}</Text>
            ...
            <Picker selectedValue={priority} onValueChange={setPriority} style={{ backgroundColor: '#EEE', marginBottom: '5%' }}>
...
const styles = StyleSheet.create({
  ...
  taskNote: { color: '#DDD', fontSize: 12, marginTop: 4 },
  addTaskText: { textAlign: 'center', color: '#FFD700', fontSize: 16, marginVertical: '5%' },
  swipeActions: { flexDirection: 'row' },
  completeButton: { backgroundColor: '#4CAF50', padding: 10, borderRadius: 5, marginRight: 5 },
  deleteButton: { backgroundColor: 'red', padding: 10, borderRadius: 5, marginRight: 5 },
  editButton: { backgroundColor: '#555', padding: 10, borderRadius: 5 },
  swipeText: { color: '#FFF' },
